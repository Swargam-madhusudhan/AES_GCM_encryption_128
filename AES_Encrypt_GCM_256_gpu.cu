...
__device__ void AES_encrypt_block_device(block_t out, const block_t in, const byte RoundKey[240])
{
    state_t state_buf;
    state_t *s = &state_buf;
    for (int i = 0; i < 4; ++i)
...
void gctr(const byte *RoundKey, const block_t ICB, const byte *in, size_t len_bits, byte *out)
{
    ...
    cudaMalloc((void **)&d_roundKey, 240);
    cudaMalloc((void **)&d_icb, 16);

    cudaMemcpy(d_in, in, total_bytes, cudaMemcpyHostToDevice);
    cudaMemcpy(d_roundKey, RoundKey, 240, cudaMemcpyHostToDevice);
