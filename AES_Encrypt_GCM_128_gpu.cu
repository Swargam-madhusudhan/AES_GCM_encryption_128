...
// GPU constant array
__device__ __constant__ byte sbox_device[256] = {
    0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
    0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0,
    0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15,
    0x04, 0xc7, 0x23, 0xc3, 0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27, 0xb2, 0x75,
...
// device version of SubBytes fucntion to use by the cuda kernel
__device__ void SubBytes_device(state_t *state)
{
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            (*state)[j][i] = sbox_device[(*state)[j][i]];
        }
    }
}
...
// device version of SubBytes fucntion to use by the cuda kernel
__device__ void AddRoundKey_device(byte round, state_t *state, const byte *RoundKey)
{
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
...
// device version of ShiftRows fucntion to use by the cuda kernel
__device__ void ShiftRows_device(state_t *state)
{
    byte temp;
    temp = (*state)[1][0];
    (*state)[1][0] = (*state)[1][1];
...
// device version of xtime fucntion to use by the cuda kernel
__device__ byte xtime_device(byte x)
{
    return ((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}
...
// device version of MixColumns fucntion to use by the cuda kernel
__device__ void MixColumns_device(state_t *state)
{
    byte a, b, c, d;
    for (int i = 0; i < 4; ++i)
    {
        ...
        c = (*state)[2][i];
        d = (*state)[3][i];
        (*state)[0][i] = xtime_device(a) ^ (xtime_device(b) ^ b) ^ c ^ d;
        (*state)[1][i] = a ^ xtime_device(b) ^ (xtime_device(c) ^ c) ^ d;
        (*state)[2][i] = a ^ b ^ xtime_device(c) ^ (xtime_device(d) ^ d);
        (*state)[3][i] = (xtime_device(a) ^ a) ^ b ^ c ^ xtime_device(d);
    }
}
...
// device version of AES_encrypt_block fucntion to use by the cuda kernel
__device__ void AES_encrypt_block_device(block_t out, const block_t in, const byte RoundKey[176])
{
    state_t state_buf;
    state_t *s = &state_buf;
    for (int i = 0; i < 4; ++i)
    ...
    AddRoundKey_device(0, s, RoundKey);
    for (int round = 1; round < Nr; ++round)
    {
        SubBytes_device(s);
        ShiftRows_device(s);
        MixColumns_device(s);
        AddRoundKey_device(round, s, RoundKey);
    }
    SubBytes_device(s);
    ShiftRows_device(s);
    AddRoundKey_device(Nr, s, RoundKey);
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
...
// device version of gcm_inc_counter fucntion to use by the cuda kernel
__device__ void gcm_inc_counter_device(block_t ctr)
{
    for (int i = 15; i >= 12; --i)
    {
        ctr[i]++;
...
// kernel code for gctr
__global__ void gctr_kernel(const byte *in, byte *out, const byte *RoundKey,
                            const byte *ICB, size_t len_bits)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;

    size_t total_bytes = (len_bits + 7) / 8;
    size_t total_blocks = (total_bytes + 15) / 16;

    if (idx >= total_blocks)
        return;

    block_t counter, enc_ctr, plain;

    memcpy(counter, ICB, 16);
    for (int i = 0; i < idx; ++i)
        gcm_inc_counter_device(counter);

    AES_encrypt_block_device(enc_ctr, counter, RoundKey);
    ...
    for (int i = 0; i < block_len; ++i)
        out[offset + i] = plain[i] ^ enc_ctr[i];

    if (len_bits % 8 != 0 && idx == total_blocks - 1)
    {
        byte mask = (0xFF << (8 - (len_bits % 8))) & 0xFF;
        out[total_bytes - 1] &= mask;
    }
}
...
void gctr(const byte *RoundKey, const block_t ICB, const byte *in, size_t len_bits, byte *out)
{
    ...
    byte *d_in = nullptr, *d_out = nullptr, *d_roundKey = nullptr, *d_icb = nullptr;

    cudaMalloc((void **)&d_in, total_bytes);
    cudaMalloc((void **)&d_out, total_bytes);
    cudaMalloc((void **)&d_roundKey, 176);
    cudaMalloc((void **)&d_icb, 16);

    cudaMemcpy(d_in, in, total_bytes, cudaMemcpyHostToDevice);
    cudaMemcpy(d_roundKey, RoundKey, 176, cudaMemcpyHostToDevice);
